...
INT CCodMakeDlg::Findprocend(ULONG FuncStartAddr,string& FuncText) {   //传入子程序地址,返回值对应不同的分析结果,1代表分析结束
	...
	UINT SigLen = 0;	//用来限定函数的字节数目为128字节
	for (;;) {
...
{	
	...
	if (m_checksource.GetCheck() == 1) {
		...
		for (int i = 0; i < pLibInfo->m_nCmdCount; i++) {
			...
			if (pCmd->m_wState == 32900) { //似乎是无效的拷贝函数
				pCmd++; pFunc++; continue;
			}
			if (pCmd->m_wState == 33028) { //似乎是无效的析构函数
				pCmd++; pFunc++; continue;
			}
			if (pCmd->m_wState == 33284) { //似乎是无效的构造函数
				pCmd++; pFunc++; continue;
			}
			...
			{
				if ((arg->m_dwState>>1) && 1 == 1)	//第二位是1,代表参数可省略
				{
					arg++;
					ECODE.append(",");
					continue;
			...
			if (ECODE[ECODE.length() - 1] == ',') {		//抹除最后一位符号
